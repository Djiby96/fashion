...
var ChildBaby = {
    ...
    page : (req, res) =>{
        ...
        `REPLACE(title, ' ', '-') AS 'titre_concat' FROM product INNER JOIN category USING(number) INNER JOIN images USING(number) ` +
        `WHERE gender='child-baby' `

        var RS = `SELECT COUNT(*) FROM product INNER JOIN category USING(number) WHERE gender='child-baby' `
        ...
        if(sex){
            R = R + `AND sub_category='${sex}' `
            RS = RS + `AND sub_category='${sex}' `  
